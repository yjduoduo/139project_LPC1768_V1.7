...
#line 29 "..\\src\\Hardware\\UART\\uart.c"
...
#line 30 "..\\src\\Hardware\\UART\\uart.c"
...
void reset_ok(void);
#line 31 "..\\src\\Hardware\\UART\\uart.c"
...
#line 32 "..\\src\\Hardware\\UART\\uart.c"
...
#line 33 "..\\src\\Hardware\\UART\\uart.c"
#line 1 "..\\src\\common\\CFlashParam.h"
...
#line 34 "..\\src\\Hardware\\UART\\uart.c"



#line 43 "..\\src\\Hardware\\UART\\uart.c"
...
void AnnCounterInit(void)
{
    ...
    vAnnCounter = M25P80_Read_1Byte((0x80000));
    if(vAnnCounter == 0xff)
        vAnnCounter = 0;

    lcd_printf("note counter:%d\n",vAnnCounter);
}
...
void NoteData_Init(void)
{
    ...
    for(i=0;i<50;i++)
    {
        ...
        lcd_printf("data2[%d]:",i);
        for(j=0;j<55;j++){
            data2[i][j]=M25P80_Read_1Byte((0x40000)+55*i+j);
            if(data2[i][0] == 0x80)
                lcd_printf("%d ",data2[i][j]);
        }
        lcd_printf("\n");
    }
...
void UART1_IRQHandler(void) 
{
    if(tp<9)
    {
        ...
        else if(tp==9)
        {
            ...
            if(CGetIntenFlag())
            {
                if(CompareAddr(data0[5],data0[4],data0[3],data0[2],0)==0)
                {
                    SetConvertdataAll();
                    UartBindSend(0x41,9);
                }
...
static void recv_note_info(void)
{
    ...
    Uart0SendByte(testann[tp3],0x00);
    tp3++;
}
...
void CSaveAnn(void)
{
    ...
    for(i=0;i<50;i++)
        for(j=0;j<55;j++)
            M25P80_Write_1Byte(((0x40000)+55*i+j),data2[i][j]);
}
...
uint32 UARTInit( uint32 PortNum, uint32 baudrate )
{
    ...
    if ( PortNum == 0 )
    {
        (*(volatile unsigned long *)(0x400FC000 + 0x0C4)) |= (1<<3);
    ...
    else if ( PortNum == 1 )
    {
        ...
        lcd_printf("uart1:pclk:%d\n",pclk);
        lcd_printf("uart1:pclk:%010ld\n",pclk);
...
{  
    ...
    if(0x02 == flag)
    {
        if( buf == '\n' )
        {
            while(0 == ((*(volatile unsigned long *)(0x4000C000 + 0x14)) & (0x20)));
            (*(volatile unsigned long *)(0x4000C000 + 0x00)) = '\r';
        }
    ...
    (*(volatile unsigned long *)(0x4000C000 + 0x00)) = buf;                    
...
void lcd_printf(char *str,...)
{
#line 703 "..\\src\\Hardware\\UART\\uart.c"
}

void DebugOnce(char *str,...)
{
#line 715 "..\\src\\Hardware\\UART\\uart.c"
}

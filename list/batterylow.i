...
#line 18 "..\\src\\Menu\\batteryLow.c"

void Battery_Low(uint8 inzone,uint8 partnumber,uint16 sum,uint8 type,PCF8563_DATE timed)
{
	uint8 i=0;
	uint8 j=0;
	uint8 AnnFlag=1;
	uint8 num=1;
	uint8 row;
	uint8 hz[]="故障总数故障欠压感烟感温手报输入输出声光";
    
    if(inzone <= get_basic_zones() && partnumber <= 50 )
	{
        ...
        Displaynumber(1,2,40,(inzone/100));
        Displaynumber(1,2,48,(inzone%100/10));
        Displaynumber(1,2,56,(inzone%100%10));
        Displaynumber(2,2,0,0x0C);
        Displaynumber(2,2,8,(partnumber%100/10));
        Displaynumber(2,2,16,(partnumber%10));
		...
		else if(type == 6)
		{
			...
			DisplayJBHZK(1,4,0,((hz[36]<<8)+hz[37]));
			DisplayJBHZK(1,4,16,((hz[38]<<8)+hz[39]));	
		}
        ...
        row = GetAnnRow(partnumber);
        if((row > 0)&&(row <= 50)&&((0xff) != row))

...
//    Debug("com1 get data:%d\n",dataCount);
    dataCount++;
    if(dataCount== RCV_SIZE)
        dataCount=0;
}
...
{
    return ((PdUartMsgUnion)data[dataCount]);
}
...
//POS_INQUIRY_BIT:for inquiry flag@uart0
//POS_ALARM_BIT :for alarm flag
...
void Init_CirAddrInfo(void)
{
    uint8 CurrentValue=0;
    uint8 i;
    Debug("init ciraddr info\n");
    for(i=COMP_START;i<=MAX_COMP;i++)
    {
        if((get_comp_ciraddr(i)>CurrentValue)&&(get_comp_ciraddr(i)<=MAX_LOOP))
        {
            Debug("set ciraddr:%d,comp:%d\n",get_comp_ciraddr(i),i);

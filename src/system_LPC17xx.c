...
#define SCS_Val               0x00000020//原来值是20
#define CLKSRCSEL_Val         0x00000001//时钟源选择
#define PLL0_SETUP            1		  //使能PLL0
#define PLL0CFG_Val           0x0000000F//BM-1后的值（PLL0经过16倍频，Fcco为384M）Fcco=2*M*Fin/N=2*16*12M/1=384M
#define PLL1_SETUP            0//禁止掉PLL1
#define PLL1CFG_Val           0x00000000//
#if 1
#define CCLKCFG_Val           0x00000003//CCLKCFG_Val+1后的值(Fcco经过4分频得到96M) cclk为96M
#else
#define CCLKCFG_Val           0x00000007//CCLKCFG_Val+1后的值(Fcco经过8分频得到48M) cclk为48M
#endif
#define USBCLKCFG_Val         0x00000007//
#define PCLKSEL0_Val          0x00111040//0xAAA2A2AA外设分频可以通过位值去选择
#define PCLKSEL1_Val          0x00000000//0xA8AAA8AA
#define PCONP_Val             0xEFEFF7DE//打开所有模块电源（原值为0x042887DE）
#define CLKOUTCFG_Val         0x00000000//禁止CLKOUTCFG引脚功能
...
uint32_t SystemFrequency = IRC_OSC; /*!< System Clock Frequency (Core Clock)  */
//此处SystemFrequency只是 一个初始化值，后面有更新， 为48M
...
void SystemInit (void)
{
    ...
    CLKSRCSEL = CLKSRCSEL_Val;    /* Select Clock Source                */
#if (PLL0_SETUP)
    PLL0CFG   = PLL0CFG_Val;
        ...
        case 1:                           /* Main oscillator => PLL0            */
            SystemFrequency = (OSC_CLK *
                               (((2 * ((PLL0STAT & 0x7FFF) + 1))) /
                                (((PLL0STAT >> 16) & 0xFF) + 1))   /
                               ((CCLKCFG & 0xFF)+ 1));
//             SFrequency = SystemFrequency;//48M=384M/8
            break;
        ...
        case 0:                           /* Internal RC oscillator => PLL0     */
        case 3:                           /* Reserved, default to Internal RC   */
            SystemFrequency = IRC_OSC / ((CCLKCFG & 0xFF)+ 1);
            break;
        case 1:                           /* Main oscillator => PLL0            */
            SystemFrequency = OSC_CLK / ((CCLKCFG & 0xFF)+ 1);
            break;
        case 2:                           /* RTC oscillator => PLL0             */
            SystemFrequency = RTC_CLK / ((CCLKCFG & 0xFF)+ 1);
            break;
        }
    }
